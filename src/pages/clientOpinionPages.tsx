...
const ClientOpinionsPage = () => {
  ...
  if (isLoading) return <p>Ładowanie recenzji...</p>;
  if (isError || !data || data.length === 0) return <p>Nie masz jeszcze żadnych recenzji.</p>;
  ...
  return (
    ...
    <div className="w-fit mx-auto mb-8">
      <button className="px-6 py-3 border-2 border-emerald-600 text-emerald-700 font-semibold rounded-lg shadow-sm hover:bg-emerald-600 hover:text-white transition-all duration-300"
          ...
          <div key={review.id} className="border p-4 rounded flex justify-between items-start">
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p>{review.comment}</p>
                <p className="text-sm text-gray-600">{new Date(review.reviewDate).toLocaleString()}</p>
              </div>
            </div>
            <button
            ...
            {isOpen && <DeleteReview setIsOpen={setIsOpen} handleDeleteReview={handleDeleteReview}/>}
          </div>
        ))}
      </div>
    </div>
