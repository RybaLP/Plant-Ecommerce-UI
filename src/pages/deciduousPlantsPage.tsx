...
const DeciduousPlantsPage = () => {
  ...
  const { data: plantsData, isLoading } = usePlantsByCategory("DECIDUOUS" as PlantType, currentPage);
  const { isAuthenticated } = useAuthenticationStore();
  const { addMutate } = useCart();
  const { addItem } = useCartStore();

  const plantsList = plantsData?.content || [];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Ładowanie roślin liściastych...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md mx-auto">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Brak roślin liściastych</h2>
          <p className="text-gray-600">Aktualnie nie mamy dostępnych roślin liściastych w ofercie.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Rośliny Liściaste</h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Odkryj naszą kolekcję pięknych roślin liściastych, które ozdobią Twój ogród przez cały rok
          </p>
        </div>
